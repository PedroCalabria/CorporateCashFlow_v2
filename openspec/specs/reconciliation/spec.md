...
Enables reconciling imported bank statement lines against ledger entries: an automatic matching engine runs after each import, entries left unmatched move to Pending Reconciliation for manual matching or a mandatory Editor justification, and a justified divergence goes through Manager approval or rejection. Also provides the manual reconciliation screen in the frontend. Backed by `LedgerEntry` and `BankStatementLine` match links, with role-based access control (Editor, Manager, Auditor) enforced by the backend as the source of truth.
