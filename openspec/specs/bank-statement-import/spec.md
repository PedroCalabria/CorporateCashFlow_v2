...
Enables importing bank statements from spreadsheets into subsidiary-scoped batches, rejecting duplicate and invalid rows at import time, listing batches scoped by role, and rejecting an entire batch with a mandatory reason. Backed by `BankStatementImportBatch` and `BankStatementLine` records, with role-based access control (Editor, Manager, Auditor) enforced by the backend as the source of truth.
