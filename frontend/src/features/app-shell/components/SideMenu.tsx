...
import { isGlobalManager, isManager, isManagerOrAuditor } from '@/features/auth/roles'
    ...
    <aside className="flex h-full w-64 flex-col border-r border-border bg-card">
      ...
      <nav className="flex-1 overflow-y-auto p-3" aria-label={t('nav.heading')}>
        ...
        <ul className="space-y-1">
          ...
          {/* Reconciliation — a real link, shown to any authenticated user (scoped server-side). */}
          <li>
            <NavLink
              to="/reconciliation"
              className={({ isActive }) =>
          ...
          {/* Audit Trail — a real link, shown only to Manager/Auditor (Editor has no access). */}
          {isManagerOrAuditor(user) && (
            <li>
              <NavLink
                to="/audit-trail"
                className={({ isActive }) =>
